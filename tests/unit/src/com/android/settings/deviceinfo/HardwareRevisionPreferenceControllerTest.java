...
public class HardwareRevisionPreferenceControllerTest {
    ...
    public void setUp() {
        ...
        mController = new HardwareRevisionPreferenceController(mMockContext);
    ...
    public void testIsShowing() throws UiObjectNotFoundException {
        ...
        if (mIsCtccVersion) {
            onView(withText("Hardware version")).check(matches(isDisplayed()));
        } else {
            onView(withText("Hardware version")).check(doesNotExist());
        }
    }
    ...
    public void testPreferenceSummary() {
        ...
        final String version = SystemProperties.get("ro.boot.hardware.revision");
