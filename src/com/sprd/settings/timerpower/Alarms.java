...
public class Alarms {
    ...
    private static final String ALARM_FLAG_FILE_PATH = "/mnt/vendor/poweron_timeinmillis";
    private static File ALARM_FLAG_FILE = new File(ALARM_FLAG_FILE_PATH);
    ...
    private static void enableAlarmInternal(final Context context,
        ...
        if (enabled) {
            ...
            // Make timing shutdown work when change TimeZone
            values.put(Alarm.Columns.ALARM_TIME, alarm.time);
        }
            ...
            if (cursor.moveToFirst()) {
                do {
                    ...
                    if (a.time == 0) {
                        ...
                        // Expired alarm, disable it and move along.
                        // ADD to check the repeat set enable.
     ...
     */
     private static void enableAlert(Context context, final Alarm alarm,
        ...
        if (alarm.label != null && alarm.label.equals("on")) {
            ...
            am.setExact(AlarmManagerWrapper.POWER_ON_WAKEUP, atTimeInMillis, sender);
            out.recycle();
        } else {
    ...
    static void disableAlert(Context context) {
        ...
        am.removeExtraAlarm(sender, null);
        intent = new Intent(TIMER_POWER_SHUTDOWN_ACTION);
        intent.setFlags(Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
        PendingIntent sender1 = PendingIntent.getBroadcast(
                context, 0, intent,
                PendingIntent.FLAG_NO_CREATE);
        am.removeExtraAlarm(sender1, null);
        alarm_flag_cancel();
    }
    ...
    /**
     * Added For Bug1056213: Reset the status of all scheduled power on/off alarms when factory reset
     */
    public static void resetAlarmStates(Context context) {
        ...
        // Delete the data file of the next scheduled power on
        if (ALARM_FLAG_FILE.exists()) {
            Log.v("cancelAllAlarms factory reset " + ALARM_FLAG_FILE + " exist");
            try {
                boolean result = ALARM_FLAG_FILE.delete();
                Log.v("cancelAllAlarms" + ALARM_FLAG_FILE + " delete success = " + result);
            } catch (Exception e) {
    ...
    private static void alarm_flag_setup(final long alarmTimeInMillis) {
        ...
        if (ALARM_FLAG_FILE.exists()) {
            ...
            try {
                boolean result = ALARM_FLAG_FILE.delete();
                Log.v(ALARM_FLAG_FILE + " delete before write,result = " + result);
            } catch (Exception e) {
        ...
        try {
            ...
            command.write(String.valueOf(timeDiffInSecs).getBytes(Charset.defaultCharset()));
            command.write("\n".getBytes(Charset.defaultCharset()));
            command.write(String.valueOf(alarmTimeInMillis / 1000).getBytes(Charset.defaultCharset()));
            command.write("\n".getBytes(Charset.defaultCharset()));
    ...
    private static void alarm_flag_cancel() {
        if (ALARM_FLAG_FILE.exists()&& bPoweron) {
            ...
            try {
                boolean result = ALARM_FLAG_FILE.delete();
                Log.v(ALARM_FLAG_FILE + " delete success = " + result);
            } catch (Exception e) {
